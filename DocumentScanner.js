...
import ScannerImages from './Images';
import {convert2PDF} from './PDFUtils';
...
export default class DocumentScanner extends PureComponent {
  ...
  onPictureProcessed = event => {
    ...
    const newList = Object.assign(this.state.imagesList);
    newList.push(event.croppedImage);
    // newList.push(event.initialImage);
  ...
  onSkip = async () => {
    const url = await convert2PDF(this.state.imagesList);
    this.props.onSkip();
    this.setState({
      showPDF: true,
      pdfLink: url,
        ...
        return (
          <View style={styles.buttonContainer}>
            <View
              ...
              <ScannerImages images={this.state.imagesList} />
              <ScannerFilters
              ...
              {this.props.hideSkip ? null : (
                <View style={[styles.buttonGroup, {marginLeft: 8}]}>
                  <TouchableOpacity
                    ...
                    onPress={cameraIsDisabled ? () => null : this.onSkip}
                    activeOpacity={0.8}>
                    <Icon
                      name="arrow-back"
                      size={40}
